...
class Cluster {
    ...
    std::vector<std::pair<int,int>> connected_children(int x, int y); // get the connected children of a node in the grid
    ...
    int directed_percolates(int percolating_index); // check if the grid percolates for directed percolation
    ...
    int percolates(); // check if the grid percolates for open boundary conditions, returns 1 if percolates, else 0
